...
public class JobController {
    ...
    public List<Job> getJobBySalary(@PathVariable(value = "salary") int salary) {
        List<Job> result = new ArrayList<>();
        for (Job job : jobs) {
            if (salary >= job.getMinSalary() && salary <= job.getMaxSalary()) {
                result.add(job);
            }
        }
        return result;
    ...
    public Job createNewJob(@RequestBody JobRequest jobRequest) {
        ...
        Job newJob = new Job(id, jobRequest.title(), jobRequest.description(), jobRequest.location(), jobRequest.minSalary(), jobRequest.maxSalary(), jobRequest.emailTo());
        jobs.add(newJob);
        return newJob;
    }
